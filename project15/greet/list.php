...
<?php
			...
			$sql="select * from pado008.greet where $find like '%$search%' order by num desc";
	} else {
		$sql="select * from pado008.greet order by num desc";
	}//if end
	try {  
		$stmh = $pdo->query($sql); 
		$count=$stmh->rowCount(); 
...
<div id="wrap">
	...
	<div id="content">
		...
		<div id="col2">
			<div class="banner"><span>KFC에 오신것을 환영합니다!</span><p>새로 오신 분들은 인사말을 남겨주세요.</p></div>
			<div id="title">
				게시글 목록보기
			</div>
			...
			?>
				<div class="list_item">
					...
					<div class="list_item2"><a href="view.php?num=<?=$item_num?>"><span><?= $item_subject ?></span></a></div>
					<div class="list_item3"><span><?= $item_nick ?></span></div>
